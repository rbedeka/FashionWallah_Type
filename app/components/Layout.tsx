  ...
  return (
    ...
    >
      <div className="flex items-center justify-center">
        <Image src={fwLogo} alt="Logo" width={'1080px'} height={'217px'} className='brand_logo'/>
      </div>
      <ContactSection />
      <FooterMenu menu={menu} />
      <div className='w-full flex flex-row items-center lg:justify-end sm:justify-between my-5 px-10'>
        <VisaCardIcon />
        <MasterCardIcon />
        <GpayIcon />
        <ShopIcon />
...
const ContactSection = () => {
  return (
    <div className="w-full flex lg:flex-row sm:flex-col justify-center items-center">
      <div className="flex flex-col justify-between items-left row_col_mx  my-3 mx-16">
        <div className="lg:text-4xl md:text-2xl sm:text-xl ">Hey there 👋🏼</div>
...
function FooterMenu({menu}: {menu?: EnhancedMenu}) {
  ...
  return (
    <div className='w-full flex justify-center items-center bg-gray-500'>
      {(menu?.items || []).map((item) => (
        <section key={item.id} className={styles.section+' text-md mx-6 lg:mx-12 text-center'} >
          <Link
            key={item.id}
            to={item.to}
            target={item.target}
